...
class MaeAutobin(nn.Module):
    def __init__(
        ...
        ##### decoder
        self.decoder = pytorchTransformerModule(max_seq_len, embed_dim, decoder_depth, heads)
        self.decoder_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)
        self.norm = nn.LayerNorm(decoder_embed_dim)
        self.to_final = nn.Linear(decoder_embed_dim, 1)
        self.padding_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
    ...
    def forward(self, x, compound, ids, pos, padding_labels, id_padding_labels, mask_labels = None, args=None, **kwargs):
        ...
        masked_x = x 
        x_pad_idx = (x==self.pad_token_id)
        print(x_pad_idx,x_pad_idx.shape)
        data_gene_ids = ids
        if mask_labels != None:
        ...
        x = self.encoder(x, padding_mask=mask_labels)
        ...
        if mask_labels != None:
            x[~mask_labels] = self.decoder_embed(x[~mask_labels])

            x[mask_labels] = self.mask_token
        else :
        ...
        #     return x.squeeze(2) 
        # else:
        return fusion_emb, x, raw_x, compound_emb
